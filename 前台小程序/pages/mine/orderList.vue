<template>
	<view class="collect">
		...
		<view class="tabs between typeTab" v-if="currentTab === 1">
			<view :class="['tabItem', currentYahooType === item.id ? 'active' : '']" 
				v-for="(item, index) in yahooTypeList" :key="index" @click="chooseYahooType(item.id)">
				{{item.title}}
			</view>
		</view>
		<view class="tabs between typeTab" v-if="currentTab === 2">
			<view :class="['tabItem', currentMercariType === item.id ? 'active' : '']" 
				v-for="(item, index) in mercariTypeList" :key="index" @click="chooseMercariType(item.id)">
				{{item.title}}
			</view>
		</view>
			...
			<view :class="['item', selectMode ? 'center selectMode' : '']"
				...
				<view class="container">
					<view class="itemHeader between">
						...
						<view class="copy" @click.stop="copyOrder(item.orderid)">复制</view>
					</view>
					<view class="itemNav flex">
						...
						<view class="info column">
							...
							<view class="bottom between">
								<view class="price">订单总价<text>{{item.totalPrice}}</text> 円</view>
							</view>
							<view class="bottom between">
								<view class="price" v-if="item.ratetype === 0">出价价格<text>{{item.price}}</text> 円</view>
								<view class="price" v-if="item.ratetype === 1">商品价格(含税)<text>{{parseInt(item.price * 110) / 100}}</text> 円</view>
							</view>
							<view class="bottom between">
								<view class="price" v-if="item.ratetype === 0 && item.nowprice">商品现价<text>{{item.nowprice}}</text> 円</view>
								<view class="price" v-if="item.ratetype === 1 && item.nowprice">商品现价(含税)<text>{{parseInt(item.nowprice * 110) / 100}}</text> 円</view>
							</view>
		...
		<view class="list" v-if="currentTab === 2">
			<view :class="['item', selectMode ? 'center selectMode' : '']"
				...
				@click="chooseItem(item)" @longpress="selectOrder(item)">
				<view class="radio" v-if="selectMode">
					<radio :checked="item.isSelect" :disabled="item.type !== 2" @click.navtive="onSelectChange(item)"/>
				</view>
				<view class="container">
					<view class="itemHeader between">
						<view class="no">订单号：{{item.orderid}}</view>
					...
					<view class="itemNav flex">
						...
						<view class="info column">
							...
							<view class="price">数量：x 1</view>
							<view class="bottom between">
								<view class="price">订单总价<text>{{item.totalPrice}}</text> 元</view>
							</view>
							<view class="bottom between">
								<view class="price">商品价格<text>{{item.price}}</text> 元</view>
							</view>
							<view class="status between">
								<view>订单状态：</view>
								<view>{{mercariTypeMap[item.type]}}</view>
							</view>
						</view>
					</view>
				</view>
					...
					<view class="container">
						...
						<view class="itemNav flex">
							...
							<view class="info column">
								...
								<view class="price">数量：x 1</view>
								<view class="bottom between">
									<view class="price">商品价格<text>{{it.price}}</text> 元</view>
								</view>
								<view class="status between">
									<view>订单状态：</view>
									<view>{{mercariTypeMap[item.type]}}</view>
				...
				<view class="options center" v-if="!selectMode">
					<view class="btn" @click.stop="showChildren(item)" v-if="item.children && item.children.length>0 && !item.showChildren">显示子订单</view>
					<view class="btn" @click.stop="showChildren(item)" v-if="item.children && item.children.length>0 && item.showChildren">隐藏子订单</view>
					<view class="btn" @click.stop="toPath('/pages/mercari/requestSend?type=2&id=' + item.id)" v-if="item.type === 2">申请发货</view>
					<view class="btn" @click.stop="showCostDetail(item)" v-if="item.type === 4">支付尾款</view>
				</view>
			</view>
			<view class="tips" v-if="list.length === 0">空空如也</view>
		</view>
