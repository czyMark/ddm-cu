...
namespace ReceiptDAL
{
    public class HttpClientHelper
    {
        ...
        //    return returnXml;
        //}
        public string Post(string data, string uri, string token = null)
        {
            string serviceUrl = $"{this.BaseUri}/{uri}";

            // 创建 HTTP 请求
            HttpWebRequest myRequest = (HttpWebRequest)WebRequest.Create(serviceUrl);

            // 设置请求方法、ContentType 等
            myRequest.Method = "POST";
            myRequest.ContentType = "application/json";
            myRequest.MaximumAutomaticRedirections = 1;
            myRequest.AllowAutoRedirect = true;
            ...
            {
                myRequest.Headers.Add("Token", token);  // 自定义 Token 头
                                                        // 或者使用标准的 Authorization 头：
